...
export default function Card ({item}){
  const {id, name, imageURL, weight, flavor, brand, price, description, whenToTake} = item
  ...
  return (
    <div className="flex flex-col px-[25px] bg-white w-[250px] rounded-xl shadow-xl text-primary">
      ...
      <p className="max-w-[200px] truncate font-bold text-center">{`${name} ${brand}`}</p>
      ...
      onClick={()=>{setProduct(name); changeProductDetails()}}>
        Ver detalhes
      </button>
    </div>
  )
